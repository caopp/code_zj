...
-(void)loadMessage:(ECMessage *)message{
    self.labelTime.text =  message.showTime? [CSPUtils changeTheDateString:[CSPUtils getTime:message.dateTime]]:@"";
    ...
    if (![skuList isKindOfClass:[NSArray class]]) {
        return;
    }
    //"skuList":[{"135\/68A":"6,0"},{},{"145\/72A":"7,0"},{}]}"msgType":"2;3;2;3",
    ...
    for (int i =0 ;i< arrColor.count;i++) {
        ...
        NSString *modeType = [arrType objectAtIndex:i];
        NSDictionary *dic = [skuList isKindOfClass:[NSArray class]]?[skuList objectAtIndex:i]:[NSDictionary new];
