...
- (NSString *)stringMonthDayH
{
    return [NSDate dateMonthDayWithDateH:self];
}
...
- (NSString *)stringToday
{
    ...
    if (chaDay == 0) {
        ...
        str =[NSString stringWithFormat:@"昨天 %@",[self stringTime]];
    }else if (chaDay == -2){
        str = [NSString stringWithFormat:@"前天 %@",[self stringTime]];
    }else{
        str = [self stringCutSeconds];
    }
...
+ (NSString *)dateMonthDayWithDateH:(NSDate *)date
{
    if (date == nil) {
        date = [NSDate date];
    }
