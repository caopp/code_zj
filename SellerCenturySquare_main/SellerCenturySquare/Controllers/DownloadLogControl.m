...
// !下载完成的通知
NSString * const NotificationOfDownloadComplete = @"NotificationOfDownloadComplete";
...
- (void)setupWithLogInfo:(NSDictionary*)logInfo {
    ...
    DebugLog(@"11下载进度更新 < %@ > < %ld > ||  进度: %.02f  ||  下载速度: 0  ||  %llu / %llu ||",self.goodsNo, (long)self.type, self.progress, self.receivedBytes, self.fileContentSize);
...
//        NSLog(@"下载状态更新 < %@ > < %ld > ||  %@ ====> %@", self.goodsNo, (long)self.type, [self convertStatusToString:self.status], [self convertStatusToString:status]);
    }
    ...
    NSLog(@"===>PostNotificationOfDownloadStatusInfo<===");
...
- (void)downloadProcessor:(MHImageDownloadProcessor *)downloadProcessor finishProcessor:(BOOL)isFinished {
    if (downloadProcessor == self.downloadProcessor ) {
        ...
        [HttpManager sendHttpRequestForGetImageDownloadCallBackListWithGoodsNo:self.goodsNo picType:self.type success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            if ([[responseDic objectForKey:CODE]isEqualToString:@"000"]) {
                ...
                //通知刷新
                [[NSNotificationCenter defaultCenter]postNotificationName:K_NOTICE_RELOADDOWNLOADCOUNT object:nil userInfo:nil];
            }
...
- (NSDictionary*)convertToDictonaryForSavingAsPlist {
    ...
    //!图片张数
    if (self.objectiveFigure.imageItems) {
    ...
    //!图片张数
    if (self.windowFigure.imageItems) {
...
- (void)logFigureUpdatedWithProgress:(CGFloat)progress averageBandwidthUsedPerSecond:(unsigned long)averageBandwidthUsedPerSecond receivedBytes:(long long)receivedBytes fileContentSize:(long long)fileContentSize figureType:(DownloadFigureType)figureType {
    ...
    DebugLog(@"22下载进度更新 < %@ > < %ld > ||  进度: %.02f  ||  下载速度: %lu  ||  %llu / %llu ||",self.goodsNo, (long)figureType, progress, averageBandwidthUsedPerSecond, receivedBytes, fileContentSize);
}
...
- (void)convertFromDictionariesInPlist:(NSArray*)dictionaries {

    for (NSDictionary* logInfo in dictionaries) {
...
- (void)loadStateFromPlist {
    ...
    //得到完整的路径名
    NSString *fileName = [plistPath1 stringByAppendingPathComponent:[NSString stringWithFormat:@"%@-%@", [LoginDTO sharedInstance].merchantNo, plistFileName]];
...
- (void)logItemDownloadedWithGoodsNo:(NSString *)goodsNo figureType:(DownloadFigureType)figureType {
    ...
    // !发出通知 下载完毕 停止下载动画
    [[NSNotificationCenter defaultCenter]postNotificationName:NotificationOfDownloadComplete object:nil];
...
-(BOOL)downloadingIsAllPause{
    ...
    for (DownloadLogItem * loadingItem in downLoadingArray) {
        ...
        DebugLog(@"%lu~~~~~~~~~%lu", (unsigned long)windowFigure.downloadProcessor.state,(unsigned long)objectFigure.downloadProcessor.state);
        
        if (!windowFigure  && windowFigure.downloadProcessor.state != DownloadProcessorStateOfStopped && windowFigure.downloadProcessor.state != DownloadProcessorStateOfInQueue) {
            
            return NO;//!不处于暂停状态，返回no
        }
        
        if (!objectFigure && objectFigure.downloadProcessor.state != DownloadProcessorStateOfStopped && objectFigure.downloadProcessor.state != DownloadProcessorStateOfInQueue) {
