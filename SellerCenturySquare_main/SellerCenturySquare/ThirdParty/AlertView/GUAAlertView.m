...
@property (nonatomic) UILabel *filterLabel;// !分割线
@property (nonatomic) UILabel *detailInfoLabel;// !详细描述
...
+ (instancetype)alertViewWithTitle:(NSString *)title  withTitleClor:(UIColor *)titleColor withTitleFont:(CGFloat)titleFont
    ...
    return [[GUAAlertView alloc]initWithTitle:title withTitleClor:titleColor withTitleFont:titleFont message:message withMessageColor:messageColor withMessageFont:messageFont withFileterLine:isWithLine withDetailInfo:detailInfo withDeatilColor:detailColor withDeatilFont:detailFont oKButtonTitle:buttonTitle withOkButtonColor:okButtonColor cancelButtonTitle:cancelButtonTitle withOkCancelColor:cancelBtnColor withView:shotView buttonTouchedAction:buttonBlock dismissAction:dismissBlock];
    
    
}

- (instancetype)initWithTitle:(NSString *)title  withTitleClor:(UIColor *)titleColor withTitleFont:(CGFloat)titleFont
                      message:(NSString *)message  withMessageColor:(UIColor *)messageColor withMessageFont:(CGFloat)messageFont
                withFileterLine:(BOOL)isWithLine
                withDetailInfo:(NSString *)detailInfo  withDeatilColor:(UIColor *)detailColor withDeatilFont:(CGFloat)detailFont
                oKButtonTitle:(NSString *)oKButtonTitle withOkButtonColor:(UIColor *)okButtonColor
            cancelButtonTitle:(NSString *)cancelButtonTitle withOkCancelColor:(UIColor *)cancelBtnColor
                     withView:(UIView *)shotView
          buttonTouchedAction:(GUAAlertViewBlock)buttonAction
                dismissAction:(GUAAlertViewBlock)dismissAction {
...
- (void)setup {
    ...
    if (_isWithLine) {
        ...
        [self setupDetailInfoLabel];
        
        [self setupLayOutWithDetailInfo];
    ...
    //!设置白色alertview部分的约束
    [self setAlertViewContrConstraint];
    
    
}
// !带详细信息的 布局
- (void)setupLayOutWithDetailInfo{
    ...
    NSDictionary *views = @{@"title": _titleLabel,
                            ...
                            @"detailInfoLabel":_detailInfoLabel,
                            @"buttonOK": _buttonOK,
    ...
    [_alertView addConstraints:[NSLayoutConstraint
                                ...
                                @"V:|-padding-[title]-[content]-[filterLabel]-[detailInfoLabel]-padding-[buttonOK(==buttonHeight)]|"
                                options:0
                                metrics:metrics
                                views:views]];
    ...
    [_alertView addConstraints:[NSLayoutConstraint
                                constraintsWithVisualFormat:
                                @"V:|-padding-[title]-[content]-[filterLabel]-[detailInfoLabel]-padding-[buttonCancel(==buttonHeight)]|"
                                options:0
                                metrics:metrics
                                views:views]];
    ...
    // !宽度
    [_alertView addConstraints:[NSLayoutConstraint
                                constraintsWithVisualFormat:@"H:|-linePadding-[filterLabel]-linePadding-|"
                                options:0
                                metrics:metrics
                                views:views]];
    //!高度
    [_alertView addConstraint:[NSLayoutConstraint
                               constraintWithItem:_filterLabel
    ...
    [_alertView addConstraint:[NSLayoutConstraint constraintWithItem:_detailInfoLabel
...
-(void)setAlertViewContrConstraint{
...
//    [self addConstraints:@[constraintCenterX, constraintCenterY, constraintLeft,constraintRight, constraintHeightMin, constraintHeightMax]];

    [self addConstraints:@[constraintCenterY, constraintLeft,constraintRight]];
...
- (void)setupContent {
    ...
    if (_messageFont) {
        ...
        label.font = [UIFont systemFontOfSize:_messageFont];
...
- (void)setupFilterLabel{
    ...
    UILabel * label = [self labelWithText:nil];
    [label setBackgroundColor:[UIColor colorWithHexValue:0x999999 alpha:1]];
    [_alertView addSubview:label];
    _filterLabel = label;
    
    
}
- (void)setupDetailInfoLabel{

    UILabel * label = [self labelWithText:_detailInfo];
    if (_detailFont) {
        
        label.font = [UIFont systemFontOfSize:_detailFont];
    ...
    _detailInfoLabel = label;
    _detailInfoLabel.numberOfLines = 0;
    [_detailInfoLabel sizeToFit];

    if (_detailColor) {
        
        [_detailInfoLabel setTextColor:_detailColor];
...
//        NSInteger length = [self.messageLabel.text length];
        ...
        [str addAttribute:NSForegroundColorAttributeName value:self.changeColor range:NSMakeRange(self.changeIndex,self.changeNum)];
