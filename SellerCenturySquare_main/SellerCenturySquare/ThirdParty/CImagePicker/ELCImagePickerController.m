...
#pragma mark --------进行根视图的设置------
- (id)initImagePicker
{
    //设置主列表的cell，图片以及个数个名字
...
{
    NSLog(@"previousCount = %lu",previousCount);
     
    BOOL shouldSelect = previousCount < self.maximumImagesCount;
    if (!shouldSelect) {
        NSString *message = [NSString stringWithFormat:NSLocalizedString(@"你最多只能选择 %d 照片.", nil), self.maximumImagesCount];
