...
#pragma mark ----增加点击触摸事件-----
        UITapGestureRecognizer *gesture = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(setNotification)];
...
#pragma mark -------设置点击触摸事件-----
-(void)setNotification
{
    if ([self.from isEqualToString:@"0"]) {
        NSDictionary *dic = @{@"isback":_isback
                              };
       
        NSNotification *nitification = [[NSNotification alloc]initWithName:@"ImgNotification" object:self userInfo:dic];
