...
@interface ZYQAssetViewController ()<ZYQAssetViewCellDelegate>{
    ...
    float headerSpace;//!每行第一个图片前面的空隙
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == ceil(self.assets.count*1.0/columns) - 1) {//!如果是显示照片的最后一行
        
        [cell bind:tempAssets selectionFilter:picker.selectionFilter minimumInteritemSpacing:minimumInteritemSpacing minimumLineSpacing:minimumLineSpacing columns:columns assetViewX:headerSpace];//!距离前面的空隙和上一行一致
        
    }else{

        headerSpace = (self.tableView.frame.size.width-kThumbnailSize.width*tempAssets.count-minimumInteritemSpacing*(tempAssets.count-1))/2;//!第一个图片距离前面的空隙
        
        [cell bind:tempAssets selectionFilter:picker.selectionFilter minimumInteritemSpacing:minimumInteritemSpacing minimumLineSpacing:minimumLineSpacing columns:columns assetViewX:headerSpace];
