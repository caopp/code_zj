...
-(void)makeMessage:(NSString *)message duration:(CGFloat)interval position:(id)position
{
    ...
    //背景
    UIView *grayBgView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HIGHT)];
...
//    [self showToast:toast duration:interval position:position];
    [self showToast:toast  withGrayView:grayBgView duration:interval position:position];
 ...
 *  采用的方法
 */
- (void)showToast:(UIView *)toast withGrayView:(UIView *)grayView duration:(CGFloat)interval position:(id)point {
