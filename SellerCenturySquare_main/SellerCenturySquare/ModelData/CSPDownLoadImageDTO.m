...
-(void)setDictFrom:(NSDictionary *)dictInfo{
    ...
    @try {
        if (self && dictInfo) {
            ...
            if ([self checkLegitimacyForData:dictInfo[@"picListSmall"]]) {
                
                self.picListSmall = dictInfo[@"picListSmall"];
            }
    ...
    @try {
        if (self && dictInfo) {
            ...
            if ([self checkLegitimacyForData:[dictInfo objectForKey:@"zipUrl"]]) {
                ...
                self.zipUrl = [self transformationData:[dictInfo objectForKey:@"zipUrl"]];
            }
            
            if ([self checkLegitimacyForData:[dictInfo objectForKey:@"picType"]]) {
                
                self.picType = [self transformationData:[dictInfo objectForKey:@"picType"]];
            }
            
            if ([self checkLegitimacyForData:[dictInfo objectForKey:@"qty"]]) {
                
                self.qty = [self transformationData:[dictInfo objectForKey:@"qty"]];
            }
            
            if ([self checkLegitimacyForData:[dictInfo objectForKey:@"picSize"]]) {
                
                self.picSize = [self transformationData:[dictInfo objectForKey:@"picSize"]];
            }
