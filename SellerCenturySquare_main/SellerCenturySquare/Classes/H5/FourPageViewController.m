...
@interface FourPageViewController ()<UIWebViewDelegate>
{
...
#pragma mark -----与h5交互写入plist文件当中-------
    ...
    [_bridge registerHandler:@"openNewPage" handler:^(id data, WVJBResponseCallback responseCallback) {
        ...
        fourPageVC.titleRecord = data[@"targetTitle"];
        fourPageVC.file = [NSString stringWithFormat:@"%@%@",[HttpManager allH5ServiceRequestWebView],data[@"targetUrl"]];
        [self.navigationController pushViewController:fourPageVC animated:YES];
    ...
    [_bridge registerHandler:@"chat" handler:^(id data, WVJBResponseCallback responseCallback) {
       ...
       // [[ChatManager shareInstance] getServerAcount:<#(NSString *)#> withName:name withController:self];
