...
@interface BQProcessView ()
@property (assign, nonatomic) NSUInteger linesOfTags;

@end

@implementation BQProcessView
...
- (instancetype)initWithWidth:(CGFloat)width contentArray:(NSMutableArray *)array {
    ...
    if (self) {
        ...
        //风格
        UILabel *artsLabel = [[UILabel alloc]initWithFrame:CGRectMake(5   , 0, 100, 11)];
        artsLabel.text = @"工艺";
...
- (void)resizeToWidth:(CGFloat)width {
    ...
    //宽度小于一个tag，不能这么玩
    for (BQTag * tag in self.tagsArray) {
...
- (NSMutableArray *)generateTagsByStrings:(NSMutableArray *)array {
    ...
    @autoreleasepool {
        for (NSString *tagString in self.contentArray) {
            BQTag *tag = [[BQTag alloc] initWithText:tagString];
            tag.index = i;
...
#pragma SEL
- (void)didTappedTag:(BQTag *)sender {
    if ([self.delegate respondsToSelector:@selector(didBQSelectTagAtIndex:)]) {
        [self.delegate didBQSelectTagAtIndex:sender.index];
    }
}
...
- (void)setTagType:(BQTagType)type {
    switch (type) {
        case BQTagTypeNormal:
            [self setNormal];
            break;
        case BQTagTypeSuccess:
            [self setSuccess];
            break;
        case BQTagTypeError:
            [self setError];
