            ...
            if ([self checkData:data class:[NSDictionary class]]) {
                ...
                if ([self checkData:orderGoodsItemsData class:[NSArray class]]) {
                    ...
                    for (NSDictionary *orderGoodsItemsDic in orderGoodsItemsData) {
                        ...
                        [self.getOrderDetailDTO.goodsList addObject:orderGoodsItem];
...
- (void)initOrderDetailsBottomView{
    ...
    if (self.orderStatus == OrderStatusWaitDeliverGoods || self.orderStatus == OrderStatusAlreadyShipped || self.orderStatus == OrderStatusTransactionComplete||self.orderStatus == OrderStatusWaitPay){
        ...
        self.orderDetailsBottomView.cancelDeliverGoodsButtonBlock = ^(){
            ...
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"确定取消交易?" message:@"与买家沟通退款后，可在此取消交易，此采购单将不计入任何与销售相关的数据统计中。" delegate:weakSelf cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        ...
        self.orderDetailsBottomView.changeOrderTotalPrice = ^()
        {
            ...
            modifyPriceView.originalTotalAmountLabel.text = [NSString stringWithFormat:@"采购单总价：￥%@",[weakSelf transformationData:self.getOrderDetailDTO.originalTotalAmount]];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    if (tableView == self.tableView) {
        if (section == 0) {
            ...
            return self.getOrderDetailDTO.goodsList.count+1;
        }
    }else{
        
        NSInteger row;
        
        if (self.getOrderDetailDTO.goodsList.count%2 == 0) {
        ...
        if (indexPath.section == 0) {
            ...
            if (indexPath.row == 0) {
                ...
                if (self.getOrderDetailDTO.status.integerValue == 0) {
                    ...
                    orderDetailsTableViewCell.orderStateLabel.text = @"采购单取消";
...
//                    orderDetailsTableViewCell.totalQuantityLabel.textColor = [UIColor lightGrayColor];
                ...
                //采购单号
                orderDetailsTableViewCell.orderCodeLabel.text = [NSString stringWithFormat:@"采购单号：%@",self.getOrderDetailDTO.orderCode];
                
                //采购单原价
                orderDetailsTableViewCell.originalTotalAmountLabel.text = [NSString stringWithFormat:@"采购单原价：￥%@",self.getOrderDetailDTO.originalTotalAmount.stringValue];
            ...
            if (indexPath.row != self.getOrderDetailDTO.goodsList.count) {
                ...
                orderGoodsItemDTO *orderGoodsItem = [self.getOrderDetailDTO.goodsList objectAtIndex:indexPath.row];
                ...
                if (self.getOrderDetailDTO.orderCancelTime.length<1) {
                    ...
                    orderCancelTime = [NSString stringWithFormat:@"采购单取消时间：%@\n\n",self.getOrderDetailDTO.orderCancelTime];
                }
                ...
                NSDictionary *attributesDic = @{NSFontAttributeName:[UIFont systemFontOfSize:11],NSForegroundColorAttributeName:[UIColor blackColor]};
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    if (buttonIndex) {
        //取消采购单/取消交易
        [self progressHUDShowWithString:@"采购单取消中"];
        ...
        [HttpManager sendHttpRequestForGetCancelOrder:self.orderCode success:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            if ([self isRequestSuccessWithCode:[responseDic objectForKey:CODE]]) {
                ...
                [self alertViewWithTitle:@"采购单取消失败" message:[responseDic objectForKey:ERRORMESSAGE]];
            }
