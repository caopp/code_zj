...
static NSString *const lastMenuString1 = @"采购单取消";
...
//采购单详情进去以后 修改或者拍照成功以后需要刷新 标记要刷新的状态 （price）价格  photo (拍照)
...
#pragma mark - 合并发货按钮 暂时放在这 以后要换地方
...
- (void)clickTotalGoodsBtn:(UIButton*)btn
{
    ...
    if (self.orderDto) {
        combinedVC.merchantNo = self.orderDto.merchantNo;
...
#pragma mark-请求全部的采购单
- (void)requestOrderListWithOrderStatus:(NSString *)orderStatus pageNo:(NSInteger)pageNo{
    
    [self progressHUDShowWithString:@"加载中"];
    
    [HttpManager sendHttpRequestForGetOrderList:orderStatus pageNo:[NSNumber numberWithInteger:pageNo] pageSize:[NSNumber numberWithInteger:PAGESIZE] channelType:[NSNumber numberWithInteger:1] success:^(AFHTTPRequestOperation *operation, id responseObject) {
        //完成上拉
        ...
        if ([self isRequestSuccessWithCode:[responseDic objectForKey:CODE]]) {
            ...
            if ([self checkData:data class:[NSDictionary class]]) {
                ...
                if ([self checkData:orderList class:[NSArray class]]) {
                    ...
                    //采购单状态(空为全部)0-采购单取消;1-未付款;2-未发货;3-已发货;4-交易取消;5-已签收
...
//        [self requestOrderListWithOrderStatus:@"1" pageNo:_pageNoForWaitPayOrder];
    ...
    if (!_isTakePhoto) {
        ...
        //待付款,待发货、待收货,采购单状态(空为全部)0-采购单取消;1-未付款;2-未发货;3-已发货;4-交易取消;5-已签收
...
- (void)initArray{
    ...
    _titleArray = [[NSMutableArray alloc]initWithObjects:@"全部",@"待付款",@"待发货",@"待收货",@"交易完成",@"采购单取消",@"交易取消", nil];
...
- (void)initTopView{
    ...
    self.myOrderTopView.chooseOrderTypeBlock = ^(NSInteger integer){
        ...
        [weakSelf    refreshControldataEableDefault];
        [weakSelf refreshControlBottomEnabledDefault];
        
        
        [((CPSMyOrderViewController *)weakSelf).scrollView setContentOffset:point animated:YES];
        
        weakSelf.menuBar = integer;
//        if (integer == MenuWaitDeliverGoodsOrder) {
...
- (void)initScrollView{
    ...
    //创建滚动视图
    self.scrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, topViewHeight+topViewInterval, self.view.frame.size.width, self.view.frame.size.height-(topViewHeight+topViewInterval-10))];
...
//        [self.myOrderTopView.btnScrollView setContentOffset:CGPointMake(self.view.frame.size.width/5.0*(scrollView.contentOffset.x/pageWidth)-self.view.frame.size.width/5.0, 0) animated:YES];
    ...
    if (self.view.frame.size.height >=tableView.contentSize.height) {
    ...
    if (array.count>0) {
       ...
       double orderTotalPrice =  [getOrder.originalTotalAmount doubleValue];
        ...
        if (getOrder.status.integerValue == 1) {
            ...
            //计算按钮的宽度
            CGFloat width = [self  gainFontWidthContent:@"拍修改采购单总价"];
            ...
            //计算按钮的宽度
            CGFloat width = [self  gainFontWidthContent:@"拍摄快递单发货"];
            
            cell.tipModifyOrTakePhoneLabel.text = @"快递单拍照后发货：";
            
            //更改按钮的宽度
            cell.shootCouriersingleDeliveryBtnWitdh.constant = width+10;
            cell.modifyOrTakePhoneButtonWitdh.constant = width+10;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    if (array.count>0) {
        ...
        orderTopView.nameWithPhoneNumLabel.text = [NSString stringWithFormat:@"%@    %@",getOrder.consigneeName,getOrder.consigneePhone];
        
        orderTopView.contactCustomerServiceBlock = ^(){
            
            DebugLog(@"联系客服");
            
            self.orderMerchantDic =getOrder.mj_keyValues;
            
            [[ChatManager shareInstance] getServerAcountWithName:getOrder.nickName withMerchanNo:getOrder.merchantNo withOrderDic:self.orderMerchantDic withController:self];
            
//            ConversationWindowViewController * conversationVC = [[ConversationWindowViewController alloc] initOrderWithName:getOrder.nickName jid:getOrder.chatAccount withMerchanNo:getOrder.merchantNo withDic:self.orderMerchantDic];
//
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (array.count >0) {
        ...
        if (indexPath.row != 0 && indexPath.row != orderGoodsItemsList.count+1) {
            ...
            orderGoodsItemDTO *orderGoodsItem = [getOrder.goodsList objectAtIndex:indexPath.row-1];
...
//            @property(nonatomic,copy)void(^changeTotalCountBlock)(NSString * orederCode,NSString * orderOldStatus,NSString * totalCount);
            ...
            detailVC.changeTotalCountBlock = ^(NSString *ordercode,NSString * orderOldStatus,NSString * totalCount)
            {
                
                NSLog(@"totalCount = %@",totalCount);
                
                CGFloat priceTotal = totalCount.doubleValue;
                NSLog(@"priceTotal = %f",priceTotal);
                
                
                getOrder.totalAmount = [NSNumber numberWithDouble:priceTotal];
                [tableView reloadData];
...
//    size=[content boundingRectWithSize:CGSizeMake(200, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName :[UIFont fontWithName:@"PingFangSC-Regular" size:13]} context:nil].size;
