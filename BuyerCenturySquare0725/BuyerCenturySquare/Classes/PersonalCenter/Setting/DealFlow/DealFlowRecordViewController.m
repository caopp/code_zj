//
//  DealFlowRecordViewController.m
//  BuyerCenturySquare
//
...
@interface DealFlowRecordViewController ()<UIWebViewDelegate>
@property WebViewJavascriptBridge* bridge;


@end

@implementation DealFlowRecordViewController
...
- (void)viewDidLoad {
   ...
   self.webView = [[UIWebView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)];
    ...
    [HttpManager paymentRecordRequestWebView:self.webView];
...
#pragma mark -----与h5交互写入plist文件当中-------
    ...
    [_bridge registerHandler:@"closeWebView" handler:^(id data, WVJBResponseCallback responseCallback) {
        ...
        if ([self.delegate respondsToSelector:@selector(removeDepositPrepaidPhoneRecordsCache)]) {
            [self.delegate performSelector:@selector(removeDepositPrepaidPhoneRecordsCache)];
        }
