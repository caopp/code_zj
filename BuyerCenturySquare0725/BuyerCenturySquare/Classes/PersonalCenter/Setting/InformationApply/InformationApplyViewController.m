//
//  InformationApplyViewController.m
//  BuyerCenturySquare
//
...
@interface InformationApplyViewController ()<UIWebViewDelegate>
@property WebViewJavascriptBridge* bridge;

@end

@implementation InformationApplyViewController
...
- (void)viewDidLoad {
    ...
    [HttpManager applicationMaterialNetworkRequestWebView:self.webView];
...
#pragma mark -----与h5交互写入plist文件当中-------
    ...
    [_bridge registerHandler:@"closeWebView" handler:^(id data, WVJBResponseCallback responseCallback) {
        ...
        if ([self.delegate respondsToSelector:@selector(removePersonalInformationRecordsCache)]) {
            [self.delegate performSelector:@selector(removePersonalInformationRecordsCache)];
        }
