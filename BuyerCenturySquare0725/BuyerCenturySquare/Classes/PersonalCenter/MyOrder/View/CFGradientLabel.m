...
- (void)drawRect:(CGRect)rect
{
    ...
    CGFloat orginX =(rect.size.width - [self accordingContentFont:self.text].width)/2.0f;
    
    CGSize textSize = [self.text sizeWithAttributes:@{NSFontAttributeName : self.font}];
    CGRect textRect = (CGRect){0, 0, rect.size.width, rect.size.height};
   
    // 画文字(不做显示用 主要作用是设置layer的mask)
    CGContextRef context = UIGraphicsGetCurrentContext();
    [self.textColor set];
    [self.text drawWithRect:CGRectMake(orginX, 0, rect.size.width, rect.size.height) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : self.font} context:NULL];
    ...
    if (!_isMainLabel) {
        mainColor = [UIColor colorWithHexValue:0x000000 alpha:1].CGColor;
        subColor = [UIColor colorWithHexValue:0x999999 alpha:1].CGColor;
    } else {
        subColor = [UIColor colorWithHexValue:0x000000 alpha:1].CGColor;
        mainColor = [UIColor colorWithHexValue:0x999999 alpha:1].CGColor;
    }
