...
- (void)drawButton {
    ...
    for (int i = 0; i <= self.nameArray.count; i++) {
        ...
        if (i == 0) {
            [btn setTitle:NSLocalizedString(@"all", @"全部") forState:UIControlStateNormal];
            ...
            GetCategoryDTO * dto = [self.nameArray objectAtIndex:i - 1];
        ...
        btn.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:12.0f];
...
- (void)btnClick:(UIButton *)sender {
    ...
    // !如果选中的是全部，那返回的id是上一级的id
    if (index == 0) {
        
        self.selectStructureNo = self.parentStructureNo;
        
    }else {
        GetCategoryDTO * dto = (GetCategoryDTO *)[self.nameArray objectAtIndex:index - 1];
