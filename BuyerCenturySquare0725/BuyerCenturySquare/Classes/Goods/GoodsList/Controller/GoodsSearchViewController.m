...
#import "SearchGoodsListView.h"//!搜索结果的list
...
-(void)createFilterAndSearchNavWithText{
    ...
    __weak GoodsSearchViewController * vc = self;
    ...
    _headerSelectView.changHightBlock = ^(BOOL isSelectBtn){//!传入参数：是否选择了“商家”/"商品"
    ...
    otherSearchView.leftLabel.text = self.searchContent;//!假搜索view 显示的内容
...
-(void)createTableView{
    ...
    //!全部collectionview
    allGoodsListView = [[SearchGoodsListView alloc]initWithFrame:CGRectMake(0, 0, self.view.frame.size.width ,self.backSrollview.frame.size.height) withQueryParam:self.searchContent withRangeFlag:@"0"];
    ...
    //!等级可看collectionview
    canBrowserGoodsListView = [[SearchGoodsListView alloc]initWithFrame:CGRectMake(CGRectGetMaxX(allGoodsListView.frame), 0, self.view.frame.size.width, self.backSrollview.frame.size.height)  withQueryParam:self.searchContent withRangeFlag:@"1"];
