...
- (void)tintColorDidChange {
    ...
    //转圈线的颜色(需要改)
    self.progressView.shapeLayer.strokeColor = HEX_COLOR(0xcb1d1dFF).CGColor;
    //边界线的颜色
...
{
    if (_tintColor == nil) {
        _tintColor = [UIColor colorWithRed: 0.0 green: 122.0/255.0 blue: 0.0 alpha: 1.0];
    }
    return _tintColor;
}
...
- (void)removeFillAnimated:(BOOL)animated {
	if (self.fillOnTouch) {
		...
		if (animated) {
			...
			highlightAnimation.toValue             = (id)[UIColor redColor].CGColor;
...
- (void)layoutSubviews {
    ...
    NSLog(@"  self.shapeLayer.strokeStart = %f," , self.shapeLayer.strokeStart);
    
}

- (UIBezierPath *)layoutPath {
    const double TWO_M_PI = 2.0 * M_PI;
    const double startAngle = 0.9 * TWO_M_PI;
